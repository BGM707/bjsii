...
function cobrosToInvoice(note: CobrosNote): Invoice {
...
export default function CobrosNotes({ prefill, onPrefillConsumed }: CobrosNotesProps) {
  ...
  const handleAutofillFromExisting = (note: CobrosNote) => {
    const { iva, total } = calcularTotales(note.neto || 0);
    // Genera un folio tentativo basado en el total de notas
    const nextFolio = `${new Date().getFullYear()}-${String(notes.length + 1).padStart(3, '0')}`;
    ...
    setForm({
      ...
      servicio_titulo: note.servicio_titulo || '',
      servicio_desc: note.servicio_desc || '',
      periodo: new Date().toISOString().slice(0, 7), // Setea automáticamente el Año-Mes actual (ej: "2026-05")
      neto: note.neto || 0,
      iva,
      total,
      estado: 'pendiente', // Nueva mensualidad inicia por defecto como pendiente
      banco: note.banco || '',
      cuenta: note.cuenta || '',
      titular: note.titular || '',
    });
    ...
    setSelectedProjectId(note.proyecto_id || '');
    setEditingNote(null); // IMPORTANTE: null le dice a Supabase que haga un INSERT (nuevo) y no un UPDATE
    setError('');
    setSuccess('');
    setViewMode('editor');
  };
    ...
    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center flex-wrap gap-3">
          <h2 className="text-2xl font-bold text-blue">Notas de Cobro</h2>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-slate-800 p-4 rounded-lg border border-slate-700">
            <p className="text-slate-400 text-xs font-bold uppercase">Total Notas</p>
            <p className="text-2xl font-black text-white">{notes.length}</p>
          </div>
          <div className="bg-red-500/10 p-4 rounded-lg border border-red-500/30">
            <p className="text-red-400 text-xs font-bold uppercase">Pendiente</p>
            <p className="text-2xl font-black text-red-400">{formatCLP(Math.round(totalPendiente))}</p>
          </div>
          <div className="bg-green-500/10 p-4 rounded-lg border border-green-500/30">
            <p className="text-green-400 text-xs font-bold uppercase">Pagado</p>
            <p className="text-2xl font-black text-green-400">{formatCLP(Math.round(totalPagado))}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-lg pl-10 pr-4 py-2 text-white text-sm focus:outline-none focus:border-blue-500"
            />
          </div>
        ...
        {showDTEGenerator && selectedNote && (
          <div className="bg-slate-800 p-6 rounded-lg border border-slate-700">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-white font-bold">Generar DTE</h3>
              <button onClick={() => setShowDTEGenerator(false)} className="text-slate-400 hover:text-white">X</button>
            </div>
            <DTEGenerator
              clientData={{
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredNotes.map((note) => {
            ...
            return (
              <div
          key={note.id}
          className="bg-slate-800 p-5 rounded-lg border border-slate-700 hover:border-slate-500 transition group"
              >
          <div className="flex justify-between items-start mb-3 gap-2">
            <div className="flex-1">
              <h3 className="text-white font-bold text-lg">#{note.folio}</h3>
              <p className="text-slate-400 text-sm">{note.cliente}</p>
              <p className="text-slate-500 text-xs">{note.rut}</p>
            </div>
            ...
            note.estado === 'pagado'
              ...
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 bg-slate-700 text-slate-400 rounded text-xs hover:bg-orange-700 transition whitespace-nowrap"
          ...
          <div className="mb-3">
            <p className="text-slate-300 text-sm font-medium">{note.servicio_titulo}</p>
          ...
          <div className="bg-slate-900 rounded-lg p-3 mb-4">
            ...
            <div className="flex justify-between text-sm font-black text-blue-400 pt-1 border-t border-slate-700">
              <span>Total</span>
          ...
          <div className="flex gap-2 flex-wrap">
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 bg-slate-700 text-white rounded text-xs hover:bg-slate-600 transition"
            >
        ...
        {filteredNotes.length === 0 && (
          <div className="text-center py-12 text-slate-400">
            <p className="text-lg">No se encontraron notas de cobro</p>
            <p className="text-sm mt-2">Crea una nueva nota o ajusta los filtros de búsqueda</p>
          </div>
        )}
            </div>
          );
        ...
        if (viewMode === 'editor') {
          return (
            <div>
        <div className="no-print max-w-6xl mx-auto bg-slate-800 p-6 rounded-b-3xl md:rounded-3xl shadow-2xl mb-10 border border-slate-700">
          <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
            <div className="flex items-center gap-3">
              <button
          onClick={() => setViewMode('list')}
          className="bg-slate-700 text-white p-2 rounded-xl hover:bg-slate-600 transition-colors"
              >
          <ArrowLeft size={18} />
              </button>
          ...
          className="bg-slate-700 text-white font-bold py-2 px-4 rounded-xl text-xs uppercase transition-all shadow-lg hover:bg-slate-600 flex items-center gap-2"
              >
          <Eye size={14} />
          Vista Previa
              </button>
          ...
          className="bg-slate-700 text-white font-bold py-2 px-4 rounded-xl text-xs uppercase transition-all shadow-lg hover:bg-slate-600 disabled:opacity-50 flex items-center gap-2"
              >
          ...
          disabled={generatingPDF}
          className="bg-green-600 text-white font-bold py-2 px-4 rounded-xl text-xs uppercase transition-all shadow-lg hover:bg-green-500 disabled:opacity-50 flex items-center gap-2"
              >
          ...
          onClick={handleSave}
          disabled={saving}
          className="bg-blue-600 text-white font-bold py-2 px-4 rounded-xl text-xs uppercase transition-all shadow-lg hover:bg-blue-500 disabled:opacity-50 flex items-center gap-2"
              >
          ...
          {error && (
            <div className="mb-4 bg-red-900/40 border border-red-700 text-red-300 text-xs font-bold rounded-lg px-4 py-3 flex items-start gap-2">
          ...
          {success && (
            <div className="mb-4 bg-green-900/40 border border-green-700 text-green-300 text-xs font-bold rounded-lg px-4 py-3">
              {success}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="space-y-3">
              <label className="text-blue-400 text-[10px] font-black uppercase tracking-widest">
          Cliente y Contacto
              </label>
              <input
          type="text"
          ...
          Detalles del Servicio
              ...
              <input
          type="text"
          placeholder="Folio (ej: 2026-001)"
          value={form.folio}
          onChange={(e) => set('folio', e.target.value)}
          className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white text-sm font-bold focus:outline-none focus:border-blue-500"
              />
              <input
          type="text"
          placeholder="Título del servicio"
          value={form.servicio_titulo}
          onChange={(e) => set('servicio_titulo', e.target.value)}
          className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white text-sm focus:outline-none focus:border-blue-500"
              />
              <textarea
          placeholder="Descripción del servicio..."
          value={form.servicio_desc}
          onChange={(e) => set('servicio_desc', e.target.value)}
          className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white text-[11px] h-14 focus:outline-none focus:border-blue-500 resize-none"
              />
            </div>
          ...
          value={form.neto || ''}
          onChange={(e) => set('neto', parseInt(e.target.value) || 0)}
          className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white text-sm font-bold focus:outline-none focus:border-blue-500"
              />
              <div className="bg-slate-900 border border-slate-700 rounded-lg overflow-hidden">
          <div className="flex justify-between px-3 py-2 text-[10px] border-b border-slate-700/60">
            <span className="text-slate-400 font-bold uppercase">Neto</span>
            <span className="text-white font-bold">{formatCLP(form.neto)}</span>
          </div>
          <div className="flex justify-between px-3 py-2 text-[10px] border-b border-slate-700/60 bg-slate-800/50">
            <span className="text-slate-400 font-bold uppercase">IVA (19%)</span>
            <span className="text-amber-400 font-bold">{formatCLP(form.iva)}</span>
          </div>
          <div className="flex justify-between px-3 py-2.5 text-[10px] bg-blue-600/20">
            <span className="text-blue-400 font-black uppercase">Total</span>
            <span className="text-blue-400 font-black text-sm">{formatCLP(form.total)}</span>
          </div>
              </div>
              <select
          value={form.estado}
          onChange={(e) => set('estado', e.target.value as 'pendiente' | 'pagado')}
          className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-xs text-white uppercase font-bold focus:outline-none focus:border-blue-500"
              >
          ...
          Cuenta de Depósito
              ...
              <input
          type="text"
          placeholder="Banco / canal"
          value={form.banco}
          onChange={(e) => set('banco', e.target.value)}
          className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-white text-[11px] focus:outline-none focus:border-blue-500"
              />
              <input
          type="text"
  ...
  return (
    <div>
      <div className="no-print max-w-6xl mx-auto bg-slate-800 p-4 rounded-xl shadow-2xl mb-6 border border-slate-700 flex justify-between items-center flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <button
            onClick={() => setViewMode('list')}
            className="bg-slate-700 text-white p-2 rounded-xl hover:bg-slate-600 transition-colors"
        ...
        {error && (
          <div className="w-full mt-2 bg-red-900/40 border border-red-700 text-red-300 text-xs font-bold rounded-lg px-4 py-3 flex items-start gap-2">
            <AlertCircle size={16} className="flex-shrink-0 mt-0.5" />
            <span>{error}</span>
          </div>
        )}
        {success && (
          <div className="w-full mt-2 bg-green-900/40 border border-green-700 text-green-300 text-xs font-bold rounded-lg px-4 py-3">
            {success}
          </div>
        )}
      </div>
