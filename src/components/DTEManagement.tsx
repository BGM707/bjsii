    ...
    <div className="space-y-6">
      <div className="bg-slate-800 rounded-lg border border-slate-700 overflow-hidden">
        ...
        <div className="p-6">
          ...
          {activeTab === 'checker' && (
            <div className="space-y-4">
              <div className="p-4 bg-cyan-500/10 border border-cyan-500/30 rounded text-cyan-300 text-sm">
          ...
          {activeTab === 'config' && (
            <div className="space-y-4">
              <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded text-amber-300 text-sm">
            ...
            <div className="space-y-4">
              {loading && (
                <div className="text-center py-8">
                  <p className="text-slate-400">Cargando DTEs...</p>
                </div>
              )}
              ...
              {!loading && dtes.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-slate-400">No hay DTEs generados</p>
                </div>
              )}
              ...
              {!loading && dtes.length > 0 && (
                <div className="space-y-2">
                  <div className="flex justify-between items-center mb-4">
                    <p className="text-slate-300 font-bold">{dtes.length} DTEs generados</p>
                    <button
                      onClick={() => loadDTEs()}
                      className="flex items-center gap-2 px-3 py-1 bg-slate-700 text-white rounded text-xs hover:bg-slate-600 transition"
                    >
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-slate-700">
                          <th className="text-left px-3 py-2 text-slate-400 font-bold">Folio</th>
                          <th className="text-left px-3 py-2 text-slate-400 font-bold">Receptor</th>
                          <th className="text-left px-3 py-2 text-slate-400 font-bold">Tipo</th>
                          <th className="text-right px-3 py-2 text-slate-400 font-bold">Total</th>
                          <th className="text-left px-3 py-2 text-slate-400 font-bold">Estado</th>
                          <th className="text-left px-3 py-2 text-slate-400 font-bold">Descargas</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-700">
                        {dtes.map(dte => (
                          <tr key={dte.id} className="hover:bg-slate-700/50">
                            <td className="px-3 py-2 text-white font-bold">{dte.folio}</td>
                            <td className="px-3 py-2 text-slate-300 text-xs">{dte.recipient_name}</td>
